...
public class PatientsController(IInsertPatientService insertPatient,
    ...
    ApplicationDbContext dbContext) : TurneroBaseController
{
    ...
    public async Task<IActionResult> Pdf(Guid? id)
    {
        ...
        var parent = await dbContext.ParentsData.AsNoTracking().SingleOrDefaultAsync(item => item.Id == id.Value);
        var personalBackground = await dbContext.PersonalBackground.AsNoTracking().SingleOrDefaultAsync(item => item.Id == id.Value);
        var perinatalBackground = await dbContext.PerinatalBackground.AsNoTracking().SingleOrDefaultAsync(item => item.Id == id.Value);
        var congenitalErrors = await dbContext.CongErrors.AsNoTracking().SingleOrDefaultAsync(item => item.Id == id.Value);
        var allergies = await dbContext.Allergies.AsNoTracking().Where(item => item.PatientId == id.Value).ToListAsync();
        var vaccines = await dbContext.Vaccines.AsNoTracking().Where(item => item.PatientId == id.Value).ToListAsync();
        var permanentMedication = await dbContext.PermMeds.AsNoTracking().Where(item => item.PatientId == id.Value).ToListAsync();
        var growthCharts = await dbContext.GrowthCharts.AsNoTracking().Where(item => item.PatientId == id.Value).ToListAsync();
        var visits = await dbContext.Visits
            .AsNoTracking()
            .Include(item => item.Medic)
            .Where(item => item.PatientId == id.Value)
            .OrderByDescending(item => item.VisitDate)
            .ToListAsync();
        var turns = new List<Turn>();
        if (long.TryParse(patient.Dni, out var patientDni))
        {
            turns = await dbContext.Turns
                .AsNoTracking()
                .Include(item => item.Medic)
